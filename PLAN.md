# Playwright-native QA Workbench PLAN.md
...
PoCではクラウドバックエンドとDBを持たず、Workbench本体はpnpm workspace、FrontendはVite + React、Local AgentはNode.js + Fastify + WebSocketで開始します。対象Playwrightプロジェクトは npm / pnpm / yarn / bun を検出して実行します。

現時点のリポジトリはテンプレート状態で、実体コードはありません。Plan Mode中のため、この内容は `PLAN.md` として保存する前提の本文ですが、今回はリポジトリ変更を行っていません。
...
- Frontend: Vite, React, TanStack Router, TanStack Query, TanStack Form, Zustand, Tailwind CSS, shadcn/ui, Monaco Editor, xterm.js。
- Agent: Node.js, TypeScript, Fastify, WebSocket, execa, ts-morph, simple-git, zod, pino。
- Reporting: Playwright JSON, Playwright HTML, allure-playwright, Allure Report 3。
- Storage: `.playwright-qa-workbench/` 配下のJSON/file store。
...
FastifyをPoCのAgentに選ぶ理由は、Node常駐Agent、zod連携、WebSocket、構造化logging、将来のOpenAPI出力と相性が良いためです。Honoは軽量候補として残しますが、初期標準にはしません。
...
1. Workbench project settingsの明示override。
2. `package.json` の `packageManager` フィールド。
3. lockfileが1種類だけ存在する場合はそのpackage manager。
4. 複数lockfileがあり、`packageManager` がない場合はambiguous warningを出し、PoCの暫定defaultは `package-lock.json` > `pnpm-lock.yaml` > `yarn.lock` > `bun.lock/bun.lockb`。GUIでoverride必須表示にする。
5. lockfileなしの場合はnpm default。ただし警告を表示する。
...
- `allure-results`: run単位で保存。
- `allure-report`: run単位でHTML生成。
- `allure-history.jsonl`: project単位の継続履歴として保存。
- `quality-gate-result.json`: exit code、失敗rule、stdout/stderr、profileを保存。
- `allure-report.csv` / `allure-log.json or text`: AI/QMOが読みやすい補助出力として保存。

Allure CLI / adapterの正確なpackage名、CLI構文、設定方法は実装直前に公式docsで再確認する。2026-04-27時点では、公式docsはAllure 3のNode install、`allurerc.*`、`historyPath`、Quality Gate、CSV/log plugin、Playwright adapterの `allure-playwright` を案内している。
...
- `apps/web`: React GUI。
- `apps/agent`: Local Node Agent。
- `packages/shared`: zod schemas、API types、domain types。
- `packages/playwright-model`: AST解析、inventory model。
...
- `docs/`: architecture notes。root `PLAN.md` は今回の計画本文の保存先候補。

PoCではpackage数を増やしすぎず、最初は `apps/web`, `apps/agent`, `packages/shared` から開始し、境界が固まったら抽出する。
...
- `NodeCommandRunner`: execa/spawnベース。
- shellは使わない。
...
- ない場合は暫定defaultを使うが、run前にoverride可能にする。
...
PoCはDBなし。対象プロジェクト配下に `.playwright-qa-workbench/` を作る設計とする。

構造案:

- `.playwright-qa-workbench/config/workbench.json`
...
- `.playwright-qa-workbench/runs/<runId>/artifacts.json`
- `.playwright-qa-workbench/runs/<runId>/ai-analysis.json`
- `.playwright-qa-workbench/runs/<runId>/patch.diff`
- `.playwright-qa-workbench/runs/<runId>/reruns/<rerunId>/...`

`.playwright-qa-workbench/` は通常git管理対象外にする。Workbenchは初回に `.gitignore` 追加diffを提案するが、自動適用しない。
...
- Allure results: `.playwright-qa-workbench/runs/<runId>/allure-results`
- Allure report: `.playwright-qa-workbench/runs/<runId>/allure-report`
- History: `.playwright-qa-workbench/reports/allure-history.jsonl`
- Known issues: `.playwright-qa-workbench/reports/known-issues.json`
- CSV/log: `.playwright-qa-workbench/runs/<runId>/allure-exports/`
...
- `playwright test --list` も併用し、実際にPlaywrightが認識するtest一覧を優先する。
...
- dirty worktreeを検出し、AI patch適用前に関連fileの状態を記録する。
- user変更を上書きしない。
- patch適用は必ず `apply --check` を先に行う。
- temporary apply後はrerunし、結果を保存してからapprove/reject判断。
- reject時は適用したpatchだけを戻す。無関係変更は触らない。
...
- dirty worktree破壊: Git状態保存とpatch単位revertに限定する。
...
- Allure resultsDirをCLI/envでrun単位に制御する最適手段。
...
4. `apps/agent` にFastify + WebSocket serverを作る。
...
「このリポジトリにpnpm workspaceを作成してください。`apps/web`, `apps/agent`, `packages/shared` の最小構成だけを追加し、Vite React、Fastify、TypeScript、zodを使える状態にしてください。まだPlaywright実行機能は作らず、health checkと型共有だけ確認してください。」

Prompt 2:

「`apps/agent` にProject scannerとPackageManagerDetectorを実装してください。project root、package.json、lockfile、playwright.config、spec候補を検出し、zod schemaで返すHTTP APIを追加してください。複数lockfile時はambiguous warningを返してください。」

Prompt 3:

「CommandRunner抽象とNodeCommandRunnerを実装してください。shellを使わずcommand/args配列で実行し、stdout/stderr streaming、exit code、timeout、cancellation、allowed command policy、audit logを実装してください。」
...
「Allure Report 3 / allure-playwrightの最新公式docsとCLI helpを確認し、PoCで使うpackage名、CLI構文、設定方法、Quality Gate実行方法を短い調査メモにまとめてください。コード変更はまだしないでください。」
...
「QMO summary v0を実装してください。Playwright JSON、Allure summary、Quality Gate、history/flaky/known issueを読み、Release Readiness SummaryをMarkdownとJSONで生成してください。」
