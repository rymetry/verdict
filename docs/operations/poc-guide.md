...
# 2. Chromium を取得 (初回のみ)
pnpm --filter @pwqa-smoke/workbench-gui exec playwright install chromium
...
CI でも同じ E2E が実行されます (`gui-smoke (Allure pipeline)` step、
`.github/workflows/ci.yml`)。失敗時は `tests/smoke/_artifacts/` に
スクリーンショットが残ります。
