...
> **重要**: 本表の `allure agent` 行と `allure log` 行は調査時点 (Context7 + 公式 docs) の結論ですが、後日 (T201 着手時) 実機 CLI で **確認した結果これらは誤りでした**。詳細は本メモ末尾の「訂正 (2026-04-29 / T201 着手時の実機検証)」セクションを参照。本表だけを引用しないでください。
...
| ~~`allure agent` コマンド~~ | ~~**存在しない** (Context7 / 公式 docs / npm 全て参照なし)~~ → **訂正: 存在する** (本メモ末尾参照) | §10 / §38 で「依存しない」と明記済み。実在確認後も方針は維持 |
...
| ~~`allure log` コマンド~~ | ~~CLI コマンドとしての記載なし。代わりに `plugin-log` (`@allurereport/plugin-log`) を `allurerc.mjs` の `plugins.log` で有効化する形~~ → **訂正: CLI コマンドである** (本メモ末尾参照) | §10 の `allure log` 出力は CLI 直接呼び出しで取得可能 |
