...
| `playwright-results.json` (Playwright JSON reporter 出力) | ✅ | `runArtifactsStore.redactPlaywrightResults` | post-write redaction; 失敗時は raw artifact を best-effort で削除し warning を残す |
...
- **対応外 artifact をデフォルトで生成しない**: 採用しない。HTML report / trace / video は **失敗調査の主要証跡** であり、生成しないと QA / SDET の根本的なワークフローを阻害する (PLAN.v2 §5 Core User Flows / §17 Failure Detail)。生成は維持し、運用者責務として手動レビューを doc 化する方が合理的。
