...
- `allure-history.jsonl`: project単位の継続履歴として `.playwright-workbench/reports/allure-history.jsonl` に保存。
...
- ない場合は**ambiguousとしてテスト実行をブロック**し、GUIでユーザーに選択を必須とする。暫定defaultによる自動決定はしない。
...
PoCはDBなし。対象プロジェクト配下に `.playwright-workbench/` を作る設計とする。
...
テスト列挙の出力形式は以下の調査順序で検証し、Phase 1標準を決定する:

1. `playwright test --list --reporter=json` — JSON出力が安定していれば、これをPhase 1標準とする。project名、日本語タイトル、記号を含むケースで検証する。
2. 最小custom reporter — JSON reporter出力が不安定な場合、test metadata（title, fullTitle, file, line, projectName, tags）のみをJSON出力する軽量custom reporterを作成する。
3. デフォルトテキスト出力のparse — **最終手段**。Playwrightの表示形式変更、project名や日本語・記号タイトルで壊れるリスクがあるため、Phase 1標準にはしない。
...
8. **AST解析**: Phase 1では `playwright test --list` を権威ソースとし、ts-morphはPhase 5/7へ延期。テスト列挙の出力形式は調査順序（JSON → custom reporter → テキスト）に従い検証後に決定。
