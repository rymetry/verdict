...
`<ThemeProvider>` 配下で `useTheme()` を使う。値は `"light" | "dark" | "auto"` の 3 値で、`auto` は `prefers-color-scheme` に追従する。`localStorage` が throw する環境 (Safari Private Mode 等) でも UI を白画面化させない安全側設計。
...
- **Issue #6 (α)** — 本 PR
- **Issue #7 (θ)** — 本 PR (foundation tests を同梱)
- Issue #8 (η): Zustand store
